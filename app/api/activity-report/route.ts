...
function toLabel(level: ActivityPayload['level']): string {
  switch (level) {
    case 'ok':
      return 'OK'
    case 'warn':
...
function buildText(p: ActivityPayload): string {
  const pcName = hostname()
  const typeText = p.type === 'page_visit' ? '📄 Sayfa Gecisi' : '⚙️ Islem'
  ...
  if (p.detail) parts.push('', `🧩 Detay: ${p.detail}`)
  return parts.join('\n')
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!tgRes.ok) return NextResponse.json({ success: false }, { status: 502 })
